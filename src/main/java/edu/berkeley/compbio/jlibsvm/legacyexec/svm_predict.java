...
// --------------------------- main() method ---------------------------
    ...
    try {
      ...
      if (predict_probability == 1) {
        if (model instanceof MultiClassModel) {
          ...
          if (!((RegressionModel) model)
              .supportsLaplace()) //svm.svm_check_probability_model(model)==0)
          {
            System.err.print("Model does not support probability estimates\n");
            System.exit(1);
          }
        ...
        if (model instanceof MultiClassModel && ((MultiClassModel) model)
            .supportsOneVsOneProbability()) {
          System.out.print("Model supports probability estimates, but disabled in prediction.\n");
        } else if (model instanceof RegressionModel && ((RegressionModel) model)
            .supportsLaplace()) {
          System.out
              .print("Model supports Laplace parameter estimation, but disabled in prediction.\n");
        }
      }
  ...
  private static void predict(BufferedReader input, DataOutputStream output, SolutionModel model,
    ...
    if (model instanceof RegressionModel) // OK we include OneClassSVC here too //svm_type == SvmParameter.EPSILON_SVR || svm_type == SvmParameter.NU_SVR)
    {
      ...
      System.out
          .print("Accuracy = " + (double) correct / total * 100 + "% (" + correct + "/" + total
              + ") (classification)\n");
    }
  }
